...
public class Grapple : MonoBehaviour {
	...
	void StartGrapple() {
		...
		dray.enabled = false; // disable the dray script to prevent player interaction
	...
	void FixedUpdate() {
		...
		case eMode.gOut: // Grappler shooting out
			...
			//   Use the TileCamera.MAP directly instead of
			//   using a collider
		...
		case eMode.gInMiss: // Grappler missed; return at double speed
			...
			if (Vector3.Dot ((p1 - p0), directions [facing]) > 0) {
				// if the grapple is still in front of Dray
				grapHead.transform.position = p1;
				grapLine.SetPosition (1, p1);
			} else {
				StopGrapple ();
